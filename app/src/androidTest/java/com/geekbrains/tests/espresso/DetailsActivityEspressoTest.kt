...
class DetailsActivityEspressoTest {
    ...
    fun activityButtonIncrement_IsWorking() {
        ...
        onView(withId(R.id.detailsTotalCountTextView)).check(matches(withText(TEST_NUMBER_OF_RESULTS_PLUS_1)))
    }

    @Test
    fun activityButtonDecrement_IsWorking() {
        onView(withId(R.id.decrementButton)).perform(click())
        onView(withId(R.id.detailsTotalCountTextView)).check(matches(withText(TEST_NUMBER_OF_RESULTS_MINUS_1)))
    }
